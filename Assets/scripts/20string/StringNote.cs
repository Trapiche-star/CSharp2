...
public class StringNote : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        str = " Abc Def Fed Cda ";  // 앞뒤에 공백 하나씩 주어서 값을 저장
        ...
        //공백 제거
        Debug.Log(str.TrimEnd());    //앞쪽에 있는 공백들, 뒤쪽 부분에 있는 공백 제거
        ...
        Debug.Log(str.Substring(5));    //5번 인덱스(방위치) 부터 끝가지 모두 가져오기    
        Debug.Log(str.Remove(5,3));     //5번 인덱스(방위치) 부터 3글자 삭제하기

        //비교
        Debug.Log(str[1] == str[15]);   //false(거짓)
        Debug.Log("ABC".Equals("Abc"));  //false(거짓)
        ...
        Debug.Log(String.Format("{0:C}", 1000));        //C 통화 단위
